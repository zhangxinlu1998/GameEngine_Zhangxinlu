        ...
        public void DrawMainGUI()
        {
            ...
            switch (ShaderReferenceEditorWindow.mPipline)
            {
                ...
                case ShaderReferenceEditorWindow.Pipline.URP:
                    ShaderReferenceUtil.DrawTitle("主平行灯");
                    ...
                    ShaderReferenceUtil.DrawOneContent("_MAIN_LIGHT_SHADOWS", "PipelineAsset中MainLight的CastShadows开启时激活.");
                    ShaderReferenceUtil.DrawOneContent("_MAIN_LIGHT_SHADOWS_CASCADE", "PipelineAsset中Shadows的Cascades不为NoCascades时激活.(阴影级联功能，移动平台不支持)");
                    ShaderReferenceUtil.DrawTitle("额外的灯");
                    ShaderReferenceUtil.DrawOneContent("_ADDITIONAL_LIGHTS", "是否额外的灯启用.");
                    ShaderReferenceUtil.DrawOneContent("_ADDITIONAL_LIGHTS_VERTEX", "PipelineAsset中AdditionalLights为PerVertex时激活.");
                    ShaderReferenceUtil.DrawTitle("烘焙GI");
                    ShaderReferenceUtil.DrawOneContent("#pragma multi_compile _ LIGHTMAP_ON", "当烘焙开启，对象勾选Contribute GI时激活.");
                    ShaderReferenceUtil.DrawOneContent("#pragma multi_compile _ _MIXED_LIGHTING_SUBTRACTIVE", "当灯光为Mixed模式，烘焙方式为Subtractive时激活.");
                    ShaderReferenceUtil.DrawOneContent("#pragma multi_compile _ DIRLIGHTMAP_COMBINED", "当烘焙DirectionalMode为Directional时激活.");
                    ShaderReferenceUtil.DrawTitle("光照贴图Lightmap");
                    ShaderReferenceUtil.DrawOneContent("说明", "光照信息存储到光照贴图时拥有不同的压缩和编码方案，这个取决于具体的平台和项目中的设置.");
                    ShaderReferenceUtil.DrawOneContent("UNITY_LIGHTMAP_FULL_HDR", "ProjectSettings>Player>LightmapEncodeing>HightQuality时生效");
                    ShaderReferenceUtil.DrawOneContent("UNITY_LIGHTMAP_RGBM_ENCODING", "ProjectSettings>Player>LightmapEncodeing>NormalQuality时生效");
                    ShaderReferenceUtil.DrawOneContent("UNITY_LIGHTMAP_DLDR_ENCODING", "ProjectSettings>Player>LightmapEncodeing>LowQuality时生效(仅在移动平台下有效,PC平台还是RGBM编码)");
                    ShaderReferenceUtil.DrawTitle("Fog 雾效");
                    ShaderReferenceUtil.DrawOneContent("unity_FogColor", "内置雾效颜色值");
